...
// VERIFY OTP and finalize registration / activation
router.post("/verify-otp", async (req, res) => {
  try {
    const {
      email,
      token, // Passed from activate.html (might be string "null" or "undefined")
      otp
    } = req.body;
    ...
    // Determine flow dynamically based on whether a password has been set.
    // If no password is set, the user is completing their invited-patient activation flow (requires set-password).
    // If a password is set, the user is self-registering.
    const isActivationFlow = !user.password;
    ...
    // If it's an activation flow, stop here so they can set password next
    if (isActivationFlow) {
      return res.json({
        success: true,
    ...
    // For direct registration, issue token and log in
